...
public class LaboratorySummaryProvider implements PatientSummaryProvider {
    ...
    public List<Summary> getSummaries(Patient patient) {
        ...
        List<Laboratory> laboratories = laboratoryRepository.findViralLoadTestByPatient(patient);
        if (laboratories.isEmpty()) {
            ...
            Laboratory laboratory = laboratories.stream()
                    ...
                    .findFirst().get();
            LaboratoryLine line = laboratoryLineRepository.findByLaboratory(laboratory).stream()
                    .filter(l-> Objects.equals(l.getLabTest().getId(),16L))
                    .findFirst().get();
