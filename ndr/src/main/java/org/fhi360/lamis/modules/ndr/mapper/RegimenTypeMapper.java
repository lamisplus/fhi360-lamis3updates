...
public class RegimenTypeMapper {
    ...
    public ConditionType regimenType(long patientId, ConditionType condition) {
        ...
        String query = "SELECT DISTINCT p.id, p.uuid, date_visit, regimen_id, regimen_type_id, duration, r.description " +
                "FROM pharmacy p JOIN pharmacy_line l ON p.id = pharmacy_id JOIN regimen_type r ON l.regimen_type_id = r.id " +
                "WHERE regimen_type_id IN (1, 2, 3, 4, 14, 5, 6, 7, 8, 9, 10, 11, 12) AND duration > 0.0 AND p.patient_id = ? " +
                "AND date_visit BETWEEN '1901-01-01' AND CURRENT_DATE AND p.archived = FALSE AND l.archived = FALSE ORDER BY " +
                "date_visit, regimen_id";
    ...
    public Date getDateRegimenStarted(long patientId, long regimenId) {
        String query = "SELECT date_visit, duration FROM pharmacy p JOIN pharmacy_line l ON p.id = pharmacy_id WHERE " +
                "p.patient_id = ? AND regimen_id = ? AND date_visit BETWEEN '1901-01-01' AND CURRENT_DATE AND p.archived = " +
                "FALSE AND l.archived = FALSE ORDER BY date_visit ASC LIMIT 1";
