...
public class EncountersTypeMapper {
    ...
    public EncountersType encounterType(long patientId) {
        ...
        encounterType.forEach(type -> {
            ...
            String query1 = "SELECT DISTINCT regimen_type_id, regimen_id, duration, p.id FROM pharmacy p JOIN pharmacy_line l " +
                    "ON p.id = pharmacy_id WHERE P.patient_id = ? AND date_visit between (to_date(?, 'YYYY-MM-DD') + INTERVAL '-7 DAY') and (to_date(?, 'YYYY-MM-DD') + " +
                    "INTERVAL '7 DAY') AND regimen_type_id IN (1, 2, 3, 4, 14) and p.archived = false and l.archived = false";
            ...
            //cotrim
            query1 = "SELECT DISTINCT description, regimen_id, duration FROM pharmacy p join pharmacy_line l on p.id = " +
                    "pharmacy_id inner join regimen_type t on l.regimen_type_id = t.id WHERE p.patient_id = ? " +
                    "AND date_visit between (to_date(?, 'YYYY-MM-DD') + INTERVAL '-7 DAY') AND (" +
                    "to_date(?, 'YYYY-MM-DD') + INTERVAL '7 DAY') AND t.id = 8 and p.archived = false and l.archived = false";
            ...
            query1 = "SELECT DISTINCT date_result_received, result FROM laboratory l JOIN laboratory_line l1 ON l.id = " +
                    "laboratory_id WHERE l.patient_id = ? AND date_sample_collected between (to_date(?, 'YYYY-MM-DD') + INTERVAL '-7 DAY') AND (" +
                    "to_date(?, 'YYYY-MM-DD') + INTERVAL '-7 DAY') AND lab_test_id = 1 and l.archived = false and l1.archived = false";
            jdbcTemplate.query(query1, rs -> {
