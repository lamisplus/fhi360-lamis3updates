...
import { CardViewDateItemModel, CardViewIntItemModel, CardViewTextItemModel, CardViewBoolItemModel, NotificationService, CoreModule } from '@alfresco/adf-core';
    ...
    PharmacyDetailsComponent.prototype.buildProperties = function () {
        ...
        this.pharmacyService.getLinesByPharmacy(this.entity.id)
            ...
            _this.properties.push(new CardViewIntItemModel({
                ...
                key: 'cs',
                value: res.map(function (r) { return r.duration; })
...
var PharmacyEditComponent = /** @class */ (function () {
    ...
    PharmacyEditComponent.prototype.ngOnInit = function () {
        ...
        this.activatedRoute.data.subscribe(function (_a) {
                ...
                _this.pharmacyService.getLinesByPharmacy(_this.entity.id)
                    ...
                    _this.rows = res.map(function (r) {
                        ...
                        _this.pharmacyService.getRegimenById(r.regimen.id).subscribe(function (res) {
                            if (!_this.regimens.map(function (r) { return r.id; }).includes(r.regimen.id)) {
                    ...
                    _this.entity.duration = res.map(function (r) { return r.duration; })
                        .sort(function (r1, r2) { return r1 - r2; })
                        .pop();
                });
            ...
            _this.pharmacyService.regimenTypes().subscribe(function (res) { return _this.regimenTypes = res; });
        });
    };
        ...
        this.selectedRegimens.forEach(function (regimen) {
            _this.pharmacyService.getDrugsByRegimen(regimen.id).subscribe(function (res) {
                res.forEach(function (drug) {
                    ...
                    if (!_this.rows.map(function (r) { return r.drug.id; }).includes(drug.drug.id)) {
                        _this.rows.push({
                            drug: drug.drug,
                    ...
                    _this.rows = _this.rows.filter(function (row) { return _this.selectedRegimens.map(function (regimen) { return regimen.id; }).includes(row.regimen.id); });
                    _this.drugs.forEach(function (drug) {
                        if (!_this.rows.map(function (r) { return r.drug.id; }).includes(drug.id)) {
                            _this.drugs = _this.drugs.filter(function (d) { return d.id !== drug.id; });
                        }
                    });
                });
            });
...
var DevolveService = /** @class */ (function () {
    ...
    DevolveService.prototype.getDevolveDatesByPatient = function (patientId) {
        return this.http.get(this.resourceUrl + "/patient/" + patientId + "/visit-dates")
            .pipe(map(function (res) {
            res.forEach(function (d) { return moment$2(d); });
            return res;
        }));
    };
