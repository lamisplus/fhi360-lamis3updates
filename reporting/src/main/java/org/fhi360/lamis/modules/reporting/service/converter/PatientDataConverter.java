...
public class PatientDataConverter {
            ...
            try {
                namedParameterJdbcTemplate.query(query, namedParameters, rs -> {
                    while (rs.next()) {
                        ...
                        cell[0] = row[0].createCell(cellnum[0]++);
                        cell[0].setCellValue((rs.getDate("date_returned_to_facility") == null) ? "" : dateFormatExcel.format(rs.getDate("date_returned_to_facility")));
                    }
                    return null;
                });
            } catch (Exception e) {
    ...
    private String getQuery() {
        return "SELECT * FROM (" +
                ...
                "               AND date_status <= CURRENT_DATE AND archived = false ORDER BY date_status DESC, id DESC LIMIT 1) date_current_status, (SELECT result FROM laboratory l " +
                "               JOIN laboratory_line ll ON l.id = laboratory_id WHERE l.patient_id = p.id AND l.archived = false AND ll.archived = false AND lab_test_id = 16 " +
                "               AND date_result_received <= CURRENT_DATE ORDER BY date_result_received DESC LIMIT 1) last_viral_load, (SELECT date_result_received FROM laboratory l " +
                "               JOIN laboratory_line ll ON l.id = laboratory_id WHERE l.patient_id = p.id AND l.archived = false AND ll.archived = false AND lab_test_id = 16 " +
                "               AND date_result_received <= CURRENT_DATE ORDER BY date_result_received DESC LIMIT 1) date_last_viral_load, (SELECT date_visit + l.duration +" +
                "               INTERVAL '28 DAYS' < CURRENT_DATE FROM pharmacy ph JOIN pharmacy_line l ON ph.id = pharmacy_id WHERE l.regimen_type_id in (1,2,3,4,14) AND ph.patient_id = p.id " +
                "               AND date_visit <= CURRENT_DATE AND ph.archived = false AND l.archived = false ORDER BY date_visit DESC, duration DESC LIMIT 1) ltfu, (SELECT date_visit " +
                "               FROM pharmacy ph JOIN pharmacy_line l ON ph.id = pharmacy_id WHERE l.regimen_type_id in (1,2,3,4,14) AND ph.patient_id = p.id" +
                "               AND date_visit <= CURRENT_DATE AND ph.archived = false AND l.archived = false ORDER BY date_visit DESC, duration DESC LIMIT 1) date_last_refill, (SELECT next_appointment " +
                "               FROM pharmacy ph JOIN pharmacy_line l ON ph.id = pharmacy_id WHERE l.regimen_type_id in (1,2,3,4,14) AND ph.patient_id = p.id " +
                "               AND date_visit <= CURRENT_DATE AND ph.archived = false AND l.archived = false ORDER BY date_visit DESC, duration DESC LIMIT 1) date_next_refill,  (SELECT duration" +
                "               FROM pharmacy ph JOIN pharmacy_line l ON ph.id = pharmacy_id WHERE l.regimen_type_id in (1,2,3,4,14) AND ph.patient_id = p.id " +
                "               AND date_visit <= CURRENT_DATE AND ph.archived = false AND l.archived = false ORDER BY date_visit DESC, duration DESC LIMIT 1) last_refill_duration," +
                "               (SELECT r.description FROM pharmacy ph JOIN pharmacy_line l ON ph.id = pharmacy_id JOIN regimen r ON r.id = regimen_id WHERE l.regimen_type_id " +
                "               IN (1,2,3,4,14) AND ph.patient_id = p.id AND date_visit <= CURRENT_DATE AND ph.archived = false AND l.archived = false ORDER BY date_visit DESC, duration DESC LIMIT 1) regimen," +
                "               (SELECT t.description FROM pharmacy ph JOIN pharmacy_line l ON ph.id = pharmacy_id JOIN regimen_type t ON t.id = regimen_type_id WHERE l.regimen_type_id " +
                "               IN (1,2,3,4,14) AND ph.patient_id = p.id AND date_visit <= CURRENT_DATE AND ph.archived = false AND l.archived = false ORDER BY date_visit DESC, duration DESC LIMIT 1) regimen_type," +
                ...
                "               (SELECT result FROM laboratory l JOIN laboratory_line ll ON l.id = laboratory_id WHERE l.patient_id = p.id AND l.archived = false AND ll.archived = false AND lab_test_id = 1 " +
                "               AND date_result_received <= CURRENT_DATE ORDER BY date_result_received DESC LIMIT 1) last_cd4, (SELECT date_result_received FROM laboratory l " +
                "               JOIN laboratory_line ll ON l.id = laboratory_id WHERE l.patient_id = p.id AND l.archived = false AND ll.archived = false AND lab_test_id = 1 " +
                "               AND date_result_received <= CURRENT_DATE ORDER BY date_result_received DESC LIMIT 1) date_last_cd4, (SELECT indication FROM laboratory l " +
                "               JOIN laboratory_line ll ON l.id = laboratory_id WHERE l.patient_id = p.id AND l.archived = false AND ll.archived = false AND lab_test_id = 16 " +
