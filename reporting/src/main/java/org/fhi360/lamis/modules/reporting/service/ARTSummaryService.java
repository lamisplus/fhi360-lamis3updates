...
public class ARTSummaryService {
    ...
    private Map<String, String> art1(Long facilityId, LocalDate start, LocalDate end) {
        ...
        String query = "SELECT * FROM (" +
                ...
                "WHERE status_at_registration != 'ART_TRANSFER_IN'";
    ...
    public Map<String, String> art2(Long facilityId, LocalDate start, LocalDate end) {
        ...
        jdbcTemplate.query(query, resultSet -> {
            while (resultSet.next()) {
                ...
                if (!exist[0]) {
                    query1 = "SELECT l.patient_id FROM laboratory l JOIN laboratory_line ll ON l.id = laboratory_id WHERE l.patient_id = ? AND " +
                            "date_result_received BETWEEN ? AND ? AND lab_test_id = 16 and l.archived = false and ll.archived = false" +
                            " ORDER BY date_result_received DESC LIMIT 1";
                    exist[0] = jdbcTemplate.query(query1, rs -> true, patientId, start, end);
                }
                if (!exist[0]) {
                    if (dateStarted != null) {
                        String query3 = "select date_visit, l.duration, date_visit + l.duration + INTERVAL '28 DAYS' < ? as ltfu, r.description regimen, " +
    ...
    public Map<String, String> art3(Long facilityId, LocalDate start, LocalDate end) {
        ...
        int[] pregnant = {0};
        int[] breastfeeding = {0};
        String query = "SELECT DISTINCT p.id, gender, DATEDIFF('YEAR', date_birth, ?) AS age FROM patient p JOIN pharmacy ph " +
                "ON p.id = patient_id JOIN pharmacy_line l ON ph.id = pharmacy_id WHERE " +
                "p.facility_id = ? AND date_started BETWEEN ? AND ? AND status_at_registration != 'ART_TRANSFER_IN' " +
                "AND ph.archived = false AND p.archived = false AND l.archived = false AND regimen_type_id IN (1, 2, 3, 4, 14)";
        jdbcTemplate.query(query, resultSet -> {
            while (resultSet.next()) {
                long patientId = resultSet.getLong("id");
                String gender = resultSet.getString("gender");
    ...
    public Map<String, String> art4(Long facilityId, LocalDate start, LocalDate end) {
        ...
        jdbcTemplate.query(query, resultSet -> {
            while (resultSet.next()) {
                ...
                String queryx = "select date_visit, l.duration, date_visit + l.duration + INTERVAL '28 DAYS' < ? as ltfu, r.description regimen, " +
                        "t.id as regimen_type_id from pharmacy p join pharmacy_line l on p.id = pharmacy_id join regimen r on r.id = l.regimen_id " +
                        "join regimen_type t on t.id = r.regimen_type_id where l.regimen_type_id in (1,2,3,4,14) and p.patient_id = ? and " +
                        "date_visit <= ? and p.archived = false and l.archived = false ORDER BY p.date_visit DESC, duration DESC LIMIT 1";
    ...
    public Map<String, String> art5(Long facilityId, LocalDate start, LocalDate end) {
        ...
        String query = "SELECT DISTINCT id, gender, DATEDIFF('YEAR', date_birth, ?) AS age FROM patient WHERE " +
                "facility_id = ? AND date_registration <= ? AND date_started <= ? AND archived = false";
        jdbcTemplate.query(query, resultSet -> {
            while (resultSet.next()) {
                long patientId = resultSet.getLong("id");
                String gender = resultSet.getString("gender");
                ...
                String query2 = "SELECT l.patient_id FROM laboratory l JOIN laboratory_line ll ON l.id = laboratory_id WHERE " +
                        "l.patient_id = ? AND date_result_received BETWEEN ? AND ? AND lab_test_id = 16 AND l.archived = false " +
                        "AND ll.archived = false ORDER BY date_result_received DESC LIMIT 1";
                jdbcTemplate.query(query2, rs -> {
                    while (rs.next()) {
                        params[0] = disaggregate(gender, age, params[0]);
                    }
    ...
    public Map<String, String> art6(Long facilityId, LocalDate start, LocalDate end) {
        ...
        jdbcTemplate.query(query, resultSet -> {
            while (resultSet.next()) {
                ...
                String query1 = "SELECT result FROM laboratory l JOIN laboratory_line ll ON l.id = laboratory_id WHERE l.patient_id = ? AND " +
                        "date_result_received BETWEEN ? AND ? AND lab_test_id = 16 AND l.archived = false AND ll.archived = false " +
                        "ORDER BY date_result_received DESC LIMIT 1";
                jdbcTemplate.query(query1, rs -> {
                    while (rs.next()) {
